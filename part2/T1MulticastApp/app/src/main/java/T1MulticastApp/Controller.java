...
public class Controller {
    ...
    public Controller() throws IOException {
        ...
        recPacket = new DatagramPacket(new byte[Model.APP_JOIN_KEY.length() + 1 + Model.APP_USER_MAX_NAME + 1],
                Model.APP_JOIN_KEY.length()+ 1 + Model.APP_USER_MAX_NAME + 1);
        sendPacket = new DatagramPacket(new byte[Model.APP_JOIN_KEY.length() + 1 + Model.APP_USER_MAX_NAME + 1],
                Model.APP_JOIN_KEY.length()+ 1 + Model.APP_USER_MAX_NAME + 1);
        groupAddress = null;
    }
    ...
    private void sendExitDatagram() throws IOException {
        sendPacket.setData((Model.APP_EXIT_KEY + " " + Model.getUserName() + "\0").getBytes());
        socket.send(sendPacket);
    }

    private void sendJoinDatagram() throws IOException {
        sendPacket.setData((Model.APP_JOIN_KEY + " " + Model.getUserName() + "\0").getBytes());
        socket.send(sendPacket);
    }

    private void sendRepeatDatagram() throws IOException {
        sendPacket.setData((Model.APP_REPEAT_KEY + " " + Model.getUserName() + "\0").getBytes());
        socket.send(sendPacket);
    }
